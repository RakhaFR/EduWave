...
import { Course, UserType, Category } from "./types";
...
interface ModalsProps {
  ...
  courseForm: { title: string; category: string; instructor: string; students: number; status: string };
  setCourseForm: (form: { title: string; category: string; instructor: string; students: number; status: string }) => void;
            ...
            <form onSubmit={handleSaveCourse} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="text"
                  required
                  placeholder="Contoh: Web Dev Dasar"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  <label className="font-bold text-slate-500">Pengajar</label>
                  <input
                    type="text"
                    required
                    placeholder="Kapten Budi"
              ...
              <div className="flex items-center justify-end gap-2.5 mt-4">
                <button
                  type="button"
                  onClick={() => setIsCourseModalOpen(false)}
                  className="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 font-bold transition-all cursor-pointer"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-xl bg-[#0073e6] hover:bg-[#0052cc] text-white font-bold transition-all shadow-md cursor-pointer"
                >
                  Simpan
                </button>
              </div>
            ...
            <form onSubmit={handleSaveCategory} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  rows={3}
                  required
                  placeholder="Deskripsi singkat seputar kategori kursus ini..."
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl p-6 w-full max-w-sm z-10 flex flex-col gap-4 text-sm transform transition-all animate-scale-in text-center">
            <div className="w-14 h-14 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-2 text-2xl font-bold">
              !
            </div>
            <div>
              <h3 className="text-base font-extrabold text-[#00172e]">Konfirmasi Hapus</h3>
              <p className="text-slate-400 mt-2 font-medium">
                Apakah Anda yakin ingin menghapus {deleteConfirm.type === "kursus" ? "kursus" : deleteConfirm.type === "kategori" ? "kategori" : "pengguna"} ini?
            ...
            <div className="flex items-center justify-center gap-3.5 mt-4">
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 font-bold transition-all cursor-pointer"
              >
                Batal
              </button>
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold transition-all shadow-md cursor-pointer"
              >
                Hapus
              </button>
            </div>
