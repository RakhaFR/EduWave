  ...
  const loadData = async () => {
    ...
    try {
      ...
      const knownIds = new Set([
        ...(friendResponse.data?.friends ?? friendResponse.friends ?? []).map((person: Person) => person.id),
        ...(requestResponse.data?.incoming ?? requestResponse.incoming ?? []).map((person: Person) => person.id),
        ...(requestResponse.data?.outgoing ?? requestResponse.outgoing ?? []).map((person: Person) => person.id),
        currentUser?.id,
      ]);
      const candidatesResponse = await courseService.searchStudyRoomUsers("");
      const candidates = candidatesResponse.data?.users ?? candidatesResponse.users ?? [];
      setSuggestedUsers(candidates.filter((person: Person) => !knownIds.has(person.id)).slice(0, 5));
    } catch (err: any) {
      setError(
              ...
              {messages.length ? (
                messages.map((item) => {
                  const own = isOwnMessage(item);
                   const sender: Person | undefined = item.sender || (own ? (currentUser ? {
                     id: currentUser.id,
                     username: currentUser.username,
                     full_name: currentUser.full_name,
                     avatar_url: currentUser.avatar_url,
                   } : undefined) : selectedChat.friend);
                  return (
                    <div key={item.id} className={`flex items-end gap-2 ${own ? "justify-end" : "justify-start"}`}>
                      {!own && <div className="h-8 w-8 shrink-0 overflow-hidden rounded-full bg-[#008be3] text-center text-xs font-bold leading-8 text-white">{avatar(sender)}</div>}
                      <div className={`max-w-[75%] rounded-2xl p-3 shadow-sm ${own ? "bg-[#008be3] text-white" : "bg-white text-slate-900"}`}>
                        <p className={`mb-1 text-[10px] font-bold ${own ? "text-white/80" : "text-[#008be3]"}`}>{own ? "Kamu" : sender?.username || selectedChat.friend.username || "Teman"}</p>
                        <p className="break-words text-sm [overflow-wrap:anywhere]">{item.content}</p>
                        <p className={`mt-1 text-[10px] ${own ? "text-white/70" : "text-slate-400"}`}>{new Date(item.sent_at).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</p>
                      </div>
                      {own && <div className="h-8 w-8 shrink-0 overflow-hidden rounded-full bg-[#008be3] text-center text-xs font-bold leading-8 text-white">{avatar((currentUser as Person | null) || undefined)}</div>}
                    </div>
                  );
                })
              ) : (
          ...
          <>
                  ...
                  {matches.map((person) => (
                    ...
                    >
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="h-10 w-10 shrink-0 overflow-hidden rounded-full bg-[#008be3] text-center font-bold leading-10 text-white">{avatar(person)}</div>
                        <div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-900">{person.full_name || person.username}</p><p className="text-xs text-slate-500">@{person.username}</p></div>
                      </div>
                      <button
            ...
            {suggestedUsers.length > 0 && (
              <section className="rounded-3xl bg-white p-5 shadow-lg">
                <h2 className="mb-1 font-extrabold text-[#00172e]">Penyelam yang disarankan</h2>
                <p className="mb-3 text-xs text-slate-500">Temukan teman belajar baru.</p>
                <div className="grid gap-2 md:grid-cols-2">{suggestedUsers.map((person) => <div key={person.id} className="flex items-center justify-between rounded-xl border border-slate-100 p-2"><div className="flex min-w-0 items-center gap-3"><div className="h-9 w-9 shrink-0 overflow-hidden rounded-full bg-[#008be3] text-center text-xs font-bold leading-9 text-white">{avatar(person)}</div><p className="truncate text-sm font-semibold text-slate-900">{person.full_name || person.username}</p></div><button onClick={() => follow(person.id)} disabled={busy} className="rounded-lg bg-[#008be3] px-3 py-2 text-xs font-bold text-white">Follow</button></div>)}</div>
              </section>
            )}
            {requests.incoming.length > 0 && (
              <section className="rounded-3xl bg-white p-5 shadow-lg">
                ...
                {requests.incoming.map((person) => (
                    ...
                    >
                      <div className="flex min-w-0 items-center gap-3"><div className="h-10 w-10 shrink-0 overflow-hidden rounded-full bg-[#008be3] text-center font-bold leading-10 text-white">{avatar(person)}</div><div><p className="text-sm font-semibold text-slate-900">{person.full_name || person.username}</p><p className="text-xs text-slate-500">@{person.username}</p></div></div>
                      <button
                      onClick={() => follow(person.id)}
                      disabled={busy}
                      className="rounded-lg bg-[#008be3] px-3 py-2 text-xs font-bold text-white"
