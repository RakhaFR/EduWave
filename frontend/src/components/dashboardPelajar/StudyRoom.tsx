...
import { Users, Plus, MessageSquare, LogOut, X, Send } from "lucide-react";
...
export default function StudyRoomComponent() {
  ...
  const leaveRoom = async () => {
    if (!selected) return;
    setBusy(true);
    try { await courseService.leaveStudyRoom(selected.id); setSelected(null); await loadRooms(); }
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-5 px-4 py-4 md:px-8 md:py-6">
        ...
        {selected ? (
          <section className="overflow-hidden rounded-3xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-100 p-4"><div><h2 className="font-extrabold text-[#00172e]">{selected.name}</h2><p className="text-xs text-slate-400">{selected.topic || "Study room"} · {selected.current_capacity}/{selected.max_capacity} peserta</p></div><button onClick={leaveRoom} disabled={busy} className="inline-flex items-center gap-1 rounded-full bg-red-50 px-3 py-2 text-xs font-bold text-red-500"><LogOut className="h-3.5 w-3.5" />Keluar</button></div>
            <div className="flex min-h-80 flex-col gap-3 bg-slate-50 p-4">{messages.length === 0 ? <p className="m-auto text-sm text-slate-400">Belum ada pesan.</p> : messages.map((item) => <div key={item.id} className="flex items-start gap-2.5"><div className="h-9 w-9 shrink-0 overflow-hidden rounded-full bg-[#008be3] text-center text-xs font-bold leading-9 text-white">{item.user?.avatar_url ? <img src={item.user.avatar_url} alt={item.user.username || "Peserta"} className="h-full w-full object-cover" /> : (item.user?.username || "P").charAt(0).toUpperCase()}</div><div className="max-w-[85%] rounded-2xl bg-white p-3 shadow-sm"><p className="text-[10px] font-bold text-[#008be3]">{item.user?.username || "Peserta"}</p><p className="text-sm text-slate-700">{item.content}</p></div></div>)}</div>
